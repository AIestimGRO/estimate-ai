"""Compare 6505382 WAW (web) vs WAM (macro)."""
...
REAL = Path("data/real")
OUT = REAL / "_compare_6505382_waw_wam.txt"
...
def main() -> None:
    wam = next(p for p in REAL.glob("*6505382*WAM*"))
    waw = next(p for p in REAL.glob("*6505382*WAW*"))
    ...
    for row in shared:
        ...
        if (len(pm) > 0) != (len(pw) > 0):
            presence_mismatch.append(
                f"row {row} {code}: macro {len(pm)} prices, web {len(pw)} prices"
            )
        elif pm and pw and pm != pw:
            price_mismatch.append(f"row {row} {code}: macro {pm[:5]} web {pw[:5]}")
