...
def _workbook_bytes(*, task_number: str) -> bytes:
    ...
    sheet.append([1, "Work A", "м", 10, "ГЭСН01-01-001-01", 100])
    sheet.append([2, "Work B", "шт", 5, "ГЭСН01-01-001-02", 200])
    sheet.append([3, "Work without unit and qty", "", "", "ГЭСН01-01-001-03", 300])
...
def test_row_preview_skips_technical_and_section_rows(tmp_path: Path) -> None:
    zip_path = tmp_path / "rnmc.zip"
    _write_zip(zip_path, {"Kazan/sections.xlsx": _section_rows_workbook_bytes()})
    ...
    assert entry.rows_rejected == 0
    assert len(entry.sample_rows) == 1
    assert entry.sample_rows[0].row_number == 7
    assert entry.sample_rows[0].work_name == "Work A"


def test_row_preview_uses_unlabeled_code_column_before_section_code(tmp_path: Path) -> None:
    zip_path = tmp_path / "rnmc.zip"
    ...
    entry = result.entries[0]
    assert entry.header_preview.code == "[без заголовка перед Код раздела]"
    assert entry.sample_rows[0].code == "ГЭСНм03-01-001-01"
    assert entry.sample_rows[0].work_name == "Work A"


def _section_rows_workbook_bytes() -> bytes:
    from io import BytesIO
    ...
    sheet.append([1, "Раздел 1. Монтажные работы", "", "", "", ""])
    sheet.append([2, "S0QUJ02CQ001-B01", "", "", "", ""])
    sheet.append([3, "Work A", "м", 2, "ГЭСН01-01-001-01", 100])
    sheet.append([None] * 6)
    sheet.append([None] * 6)
    sheet.append([None] * 6)
    output = BytesIO()
