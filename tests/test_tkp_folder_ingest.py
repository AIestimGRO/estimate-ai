...
def test_original_kl_parser_finds_structural_sheet_and_winner(tmp_path) -> None:
    ...
    assert blocks["1.3"].value == "1234567"
...
def test_original_kl_parser_normalizes_hash_prefixed_task_number(tmp_path) -> None:
    source = tmp_path / "hash-task.xlsx"
    _write_source(source, task_no="#6692713")

    result = parse_tkp_source_workbook(source)

    assert result.files[0].parse_status == "OK"
    assert result.files[0].task_no == "6692713"
    assert result.items[0].task_no == "6692713"
    blocks = {block.code: block for block in result.diagnostics[0].blocks}
    assert blocks["1.3"].value == "#6692713"


def test_original_kl_parser_flags_invalid_task_number_for_manual_review(tmp_path) -> None:
    source = tmp_path / "invalid-task.xlsx"
    _write_source(source, task_no="58420033")

    result = parse_tkp_source_workbook(source)

    assert result.files[0].parse_status == "Needs review"
    assert result.files[0].task_no == ""
    assert result.items[0].task_no == ""
    assert "Task number not found or invalid." in result.files[0].parse_message
    blocks = {block.code: block for block in result.diagnostics[0].blocks}
    assert blocks["1.3"].value == "58420033"
...
def test_admin_uploads_original_kl_folder_through_staged_preview(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(create_app(base_dir=tmp_path / "work")) as client:
        ...
        assert 'value="1234567"' in preview.text
    ...
    try:
        ...
        assert sources[0].task_no == "1234567"
    finally:
        connection.close()
...
def test_admin_missing_task_can_be_filled_before_tkp_commit(tmp_path, monkeypatch) -> None:
    ...
    try:
        rows = list_tkp_items(connection, limit=10)
        assert len(rows) == 1
        assert rows[0].task_no == "7770001"
        sources = list_tkp_sources(connection)
        assert sources[0].task_no == "7770001"
